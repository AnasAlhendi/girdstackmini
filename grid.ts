...
export interface GridStackOptions {
  ...
  bottomBar?: boolean | BottomBarOptions;
}
...
interface ParsedSize {
  n: number;
  u: string; // css unit, e.g. 'px', '%'
}

interface NormalizedBottomBar {
  columns: number;
  rows: number;
  cellHeight: number; // numeric value
  cellHeightUnit?: string; // unit for cellHeight
  margin: string; // normalized as css string
  lineHeight: number;
  color: string;
}
...
function assignShallow<T extends object, U extends object>(target: T, src: U): T & U {
  for (const k in src as any) {
    if (hasOwn(src, k)) (target as any)[k] = (src as any)[k];
  }
  return target as T & U;
}

function mergeShallow<A extends object, B extends object>(a: A, b: B): A & B {
  return assignShallow(assignShallow({} as any, a), b);
}

function toNumberMaybe(v: unknown): number | undefined {
  if (v == null || v === '') return undefined;
  const n = Number(v as any);
  return isNaN(n as any) ? undefined : n;
}

function parseSize(v: number | string | undefined): ParsedSize {
...
function normalizeBottomBar(v: boolean | BottomBarOptions | undefined): NormalizedBottomBar | null {
  if (!v) return null;
  if (v === true) {
...
function toPxValue(n: number, u?: string): number {
  const unit = u || 'px';
...
function ensureContentWrapper(el: HTMLElement): HTMLElement | null {
  const content = el.querySelector('.grid-stack-item-content') as HTMLElement | null;
  if (content) return content;
  if ((el as any).dataset && (el as any).dataset.gsNowrap === 'true') return null;
...
export class GridStack {
  ...
  public rows?: number;
  private cellHeightValue: number;
  private cellHeightUnit: string;
  public margin: string;
  private bottomBarCfg: NormalizedBottomBar | null;
  private _items: GridItem[] = [];
  private styleEl: HTMLStyleElement | null = null;
  private _onResize: (() => void) | null = null;
  private bottomBarEl?: HTMLDivElement;
  private bottomBarContentEl?: HTMLDivElement;
  private bottomBarLineEl?: HTMLDivElement;
  private bottomGrid?: GridStack;

  constructor(container: string | HTMLElement, opts: GridStackOptions = {}) {
    const el = typeof container === 'string' ? (document.querySelector(container) as HTMLElement | null) : container;
    if (!el) throw new Error('GridStack: container not found');

    const d: GridStackOptions = {
      columns: 12,
      rows: undefined as any,
      margin: 0,
      cellHeight: 80,
      bottomBar: undefined as any,
    } as any;
    opts = mergeShallow(d as any, opts as any);
    ...
    if (this.bottomBarCfg) {
      this.bottomGrid = new GridStack(this.bottomBarContentEl as HTMLElement, {
  ...
  destroy(): void {
    ...
    if (this.bottomGrid && (this.bottomGrid as any).destroy) this.bottomGrid.destroy();
    if (this.bottomBarEl && this.bottomBarEl.parentNode) this.bottomBarEl.parentNode.removeChild(this.bottomBarEl);
  }

  private _resolveItemEl(input: string | Element): HTMLElement | null {
    ...
    if (!el.classList.contains('grid-stack-item')) {
      el = (el as HTMLElement).closest('.grid-stack-item');
    }
    if (!el || !this.el.contains(el)) return null;
    return el as HTMLElement;
  }

  private _autoInitExisting(): void {
    const nodes = Array.from(this.el.children).filter((c): c is HTMLElement => (c as HTMLElement).classList.contains('grid-stack-item'));
    nodes.forEach((el) => {
  ...
  addWidget(elOrHtmlOrOpts: string | Element | WidgetOptions, opts?: WidgetOptions): HTMLElement | null {
    let el: HTMLElement | null = null;
    let conf: WidgetOptions = {};

    if (typeof elOrHtmlOrOpts === 'string') {
      const t = document.createElement('template');
      t.innerHTML = elOrHtmlOrOpts.trim();
      el = t.content.firstElementChild as HTMLElement | null;
    ...
    if (existing) assignShallow(existing as any, { x, y, w, h });
    else this._items.push({ el, x, y, w, h } as GridItem);

    this.layout();
    return el;
  }

  addBottomWidget(elOrHtmlOrOpts: string | Element | WidgetOptions, opts?: WidgetOptions): HTMLElement | null {
    if (!this.bottomGrid) return null;
    const el = this.bottomGrid.addWidget(elOrHtmlOrOpts as any, opts as any);
    this.layout();
    return el;
  }

  addWidgetHTML(html: string, xOrOpts?: number | { x?: number; y?: number; w?: number; h?: number }, y?: number, w?: number, h?: number): HTMLElement | null {
    ...
    (el as any).dataset.gsNowrap = 'true';
    let conf: { x?: number; y?: number; w?: number; h?: number } = { w: 1, h: 1 };
    if (typeof xOrOpts === 'object' && xOrOpts !== null) {
      conf = assignShallow(conf as any, xOrOpts as any);
    } else if (xOrOpts != null) {
      conf.x = toNumberMaybe(xOrOpts) as number;
  ...
  addBottomWidgetHTML(html: string, xOrOpts?: number | { x?: number; y?: number; w?: number; h?: number }, y?: number, w?: number, h?: number): HTMLElement | null {
    if (!this.bottomGrid) return null;
    const el = this.bottomGrid.addWidgetHTML(html, xOrOpts as any, y as any, w as any, h as any);
    this.layout();
    return el;
  }

  removeBottomWidget(target: string | Element): boolean {
  ...
  setCellHeight(v: number | string): void {
    const p = parseSize(v);
  ...
  layout(): void {
    ...
    if (this.bottomBarCfg) {
      ...
      const barS = (this.bottomBarEl as HTMLElement).style;
      barS.height = `${barH}px`;
      const contentS = (this.bottomBarContentEl as HTMLElement).style;
      contentS.height = `${contentH}px`;
      contentS.top = `${lineH}px`;
      const lineS = (this.bottomBarLineEl as HTMLElement).style;
  ...
  private _intersects(a: { x: number; y: number; w: number; h: number }, b: { x: number; y: number; w: number; h: number }): boolean {
    return !(a.y >= b.y + b.h || a.y + a.h <= b.y || a.x + a.w <= b.x || a.x >= b.x + b.w);
  }

  private _isAreaFree(x: number, y: number, w: number, h: number): boolean {
    const rect = { x, y, w, h };
  ...
  private _findEmptyPosition(w: number, h: number): { x: number; y: number } | null {
    const cols = Math.max(1, this.columns || 12);
    const maxY = this.rows != null ? Math.max(0, (this.rows as number) - h) : this._computedRows() + 200;
  ...
  removeWidget(target: string | Element): boolean {
  ...
  private _applyItemLayout(n: GridItem, cw: number, ch: number, gap: number): void {
    ...
    s.width = `${width}px`;
    s.height = `${height}px`;
    const content = n.el.querySelector('.grid-stack-item-content') as HTMLElement | null;
    if (content) content.style.padding = '0px'; else (n.el as HTMLElement).style.padding = '0px';
  }

  private _cellWidth(gapPx: number): number {
  ...
  private _gapPxFrom(v: number | string): number {
    const p = parseSize(v as any);
    return toPxValue(p.n, p.u || 'px');
  }

  private _ensureBottomBar(): void {
...
// Attach to global for parity with UMD usage in example.html
declare const self: any;
declare const window: any;
const rootObj: any = typeof self !== 'undefined' ? self : typeof window !== 'undefined' ? window : undefined;
